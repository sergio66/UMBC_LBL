...
C
c
cs    5 WRITE (*,'(8x,A,/,8x,A////////////)')
cs     + '   This program calculates the Total Internal Partition Sum'
cs     + ,'       for the molecular species on the HITRAN database.'
c
   10 continue
C
C
cs      WRITE (*,'(///A//)')
cs     + '   ENTER Molecule number, the choices are:'
cs      WRITE (*,'(6(I4,''= '',A6)/)') (J,MOLID(J),J=1,NMOL)
cs      WRITE (*,'(//A//)')
cs     +'     99 ends the list'
cs   20  READ (*,*,err=21) MOL
cs       go to 22
cs   21 write(*,'(a)') '   Incorrect DATA type, try again'
cs      go to 20
C
cs  22  IF (MOL .EQ. 99) THEN
cs         go to 99
cs       END IF
C
cs      IF (MOL.GT.NMOL .OR. MOL.LT.1) THEN
cs         WRITE (*,'(25X,A)')
cs    +    'Incorrect molecule assignment, try again'
cs         GO TO 20
cs      END IF
c
C
...
c
cs   30  WRITE (*,'(//A,/,A/)')
cs     +'    Enter Temperature for calculation of Q(T)',
cs     +'    Current range is 70 - 3000 K'
cs       READ (*,*,err=31) Temp
cs       go to 32
cs   31 write(*,'(a)') '   Incorrect DATA type, try again'
cs      go to 30
c
cs   32 IF (Temp .GT. 3000. .OR. Temp .LT. 70.) THEN
cs        WRITE (*,'(25X,A)') 'Temperature not in Range, try again'
cs        GO TO 30
cs      END IF
...
c 
cs
cs 100  if(QT .gt. 0.) then
cs        Write(*,900) MOLID(Mol), NSO82, Temp, QT, gi
cs  900 format(//,5x,A6,' Molecule',3x,i5,' Isotopic species',/,
cs     + 5x,'Q(',f7.1,' K) = ',1pE12.6,5x,'State Independent factor=',
cs     + 0pf7.1,////)
cs      else
cs        write(*,'(A23,A6)') ' No data available for ',MOLID(MOL)
cs      endif
      
 100  CONTINUE
cs      go to 10
cs  99  STOP'  Computation finished'
      END 
