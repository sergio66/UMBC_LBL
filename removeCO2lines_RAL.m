function [line,num_band,PQR,bandtype]=...
     removeCO2lines(band,lineORIG,low,high,num_band,PQR,bandtype,...
                    vers,strengthM,homepath,hitlin_fname,xfar); 

global quiet 

%to compute the 4 um co2 spectra we essentially have
%k = k_strongmix + k_weakmix + k_otherlines = S + W + T
%where the strong mix lines are the main PR sigsig, R pipi, R deltdelt bands
%so if we run run6co2 with the following combos :
%%
...
%
%THIS FUNCTION USED WHEN COMPUTING BACKGROUND FOR RAL DATA FITS
%RUNCO2 HAD THE fast (run6 voigt loop over lines) TURNED ON, AND
%THE LINE MIXING LOOP TURNED OFF
%
%NOTE IndicesToSearch=[-11 11 12 13];
%AND THAT WHEN YOU LOOK AT +/-11, 12,13 WE ONLY HAD THE MAIN LINES CHOSEN
%
% SO WHEN RUN6CO2 IS CALLED, ONLY THE MAIN sigsig, pipi, deltdelt lines
% ARE "REMOVED" .. WHEN THE MAIN LOOP IS CALLED, THE REMAINING LINES ARE USED 
% IN THE BACKGROUND COMPUTATION. THEN SINCE THE  LINE MIXING LOOP IS OFF,
% THE MAIN sigsig, pipi and deltdelt lines are not computed .. as described 
% above ... RUN1.m can then do data fits by calling appropriate codes !!
...
if (sum(ismember(ident,IndicesToSearch)) == 1)
  iii = iii+1;
  Q_sigpi        =  [618 648 662 667 720 791 2080];
  Q_sigpi_lower  =  [2   1   1   1   2   3   1   ];
  Q_sigpi_upper  =  [3   2   2   2   5   8   8   ];
  Q_isotope      =  [1   2   3   1   1   1   1   ];
...
for iii=1:length(IndicesToSearch)
  ...
  ID=str.ID(iii,1);                         %code to tell me which type of band
...
%%%%%      ( (PQRtype == 'R'))
  ...
  if ((length(intersect(band,bbb)) == 1) & (low <= band) & (band  <= high))
    iFound = 1;          %somethin found

%    fprintf(1,'\n found a band !! \n');

%%%%%%%%% no!!!!!!!!    fname1=[homepath hitlin_fname];
    fname1=[hitlin_fname];
    tempPQRlines=makeDAVEhitlin(band,vers,strengthM,homepath,fname1,xfar);
    ...
    %plot(v_lower(index2)); title('v_l(ind2)'); pause; size(index2)
    index3=find(v_upper==v_u);   
    %plot(v_upper(index3)); title('v_u(ind3)');pause; size(index3)
    index4=find(iso==isotype);   
    %plot(iso(index4));     title('iso(ind4)');pause; size(index4)
    zintersect=intersect(intersect(intersect(index1,index2),index3),index4);
    index=zintersect;

%   now find the 50 strongest lines from here!!!!!!!!!
    find_strongest=-1;
...
%    size(zintersect)
%    index=find((j_PQR==PQRtype) & (v_lower==v_l) & (v_upper==v_u) & ...
%                (iso==isotype));
...
%    plot(index,lineORIG.wnum(index),'+'); 
%    title('these are the non branch LINES left alone');
% fprintf(1,'\n num of orig lines, remain lines = %3i %3i \n',lineORIG.linct,length(index));

    line.linct   = sum(linctindex); 
%    line.wnumhx  = lineORIG.wnumhx;
%    line.lstat  = lineORIG.lstat(index); 
    ...
    mmm=1:length(index);
    line.uslq(mmm,1:9)   = lineORIG.uslq(index(mmm),1:9);
    line.bslq(mmm,1:9)   = lineORIG.bslq(index(mmm),1:9); 
    line.ai(mmm,1:3)   = lineORIG.ai(index(mmm),1:3); 
    line.ref(mmm,1:6)  = lineORIG.ref(index(mmm),1:6);
