      ...
      implicit none
      character*32 fname
      integer nnn,mol(19),iso(19),i,case,nlevels,j
      integer ntrans(0:1500000)
      real*4 trans(0:1500000)
      real*8 uu(99,0:29),fmix(99),ptot,grav,u0(99),dv,tau,utot(99)
      real*8 ntot,v1,v2,p0(0:29),t0(0:29),vv,taulast,gaircor
      real*8 psurf,tsurf,planck,v,t,rad,dexp,tave,enrich(19)
c
      planck(v,t)=1.1910439d-12*v*v*v/(exp(1.438679d0*v/t)-1.e0)
c
      ...
      read(11,*) case
c
      if (case.eq.1) then        ! single (p,T)
         read(11,*) v1,v2,dv
         read(11,*) gaircor
         read(11,*) p0(0),t0(0),grav
         read(11,*) nnn
         do i=1,nnn
            read(11,*) mol(i),iso(i),u0(i)
         end do
         nlevels=0
         ptot=p0(0)
         p0(1)=0.d0
         t0(1)=t0(0)
      else if (case.eq.2) then   ! T(p) profile
         read(11,*) v1,v2,dv
         read(11,*) gaircor
         read(11,*) psurf,tsurf,grav
         read(11,*) nnn
         do i=1,nnn
            read(11,*) mol(i),iso(i),u0(i)
         end do
         ...
         ptot=psurf
         p0(nlevels+1)=0.d0
         t0(nlevels+1)=t0(nlevels)
         p0(0)=psurf
         t0(0)=t0(1)+(t0(2)-t0(1))*(log(psurf)-log(p0(1)))/
     +                              (log(p0(2))-log(p0(1)))
      end if
      close(11)
      ntot=ptot*1.d3/(grav*44.d0*1.6605d-24)
      if (((case.eq.1).and.(v1+1500000.d0*dv.lt.v2)).or.
     +    ((case.eq.2).and.(v1+40000.d0*dv.lt.v2))) then
         write(*,*) 'Too many output points. Reduce spectral range'
         write(*,*) '   or increase dv for output'
         stop
      end if
c
c  Correction terms
c
      if (gaircor.gt.1.d-6) write(*,'(a,f9.4)')
     +    'All foreign broadening coefficients (gair) will '//
     +    'be set to',gaircor
      if (gaircor.lt.-1.d-6) write(*,'(a,f9.4)')
     +    'All foreign broadening coefficients (gair) will '//
     +    'be multiplied by',-gaircor
c
c  Mixing ratios
c
      do i=1,99
      ...
      write(*,*) 'mol, iso, fmix, enrich, utot'
      do i=1,nnn
         if (fmix(mol(i)).lt.0.d0) then
            enrich(i)=1.d0
            if (u0(i).gt.0.d0) then
               utot(mol(i))=u0(i)
            else
               utot(mol(i))=-u0(i)*ntot
            end if
         else
            enrich(i)=u0(i)
         end if
         fmix(mol(i))=utot(mol(i))/ntot
         do j=0,nlevels
            uu(mol(i),j)=utot(mol(i))*(p0(j)-p0(j+1))/ptot
         end do
         write(*,'(2i4,f9.6,f9.5,1pe12.4)') 
     +           mol(i),iso(i),fmix(mol(i)),enrich(i),utot(mol(i))
      end do
c
c  Initialize transmittance arrays
c
      do i=0,1500000
         trans(i)=0.
         ntrans(i)=0
...
c  Do all the work
c
      call getlinebyline(case,nnn,mol,iso,uu,fmix,enrich,
     +      v1,v2,dv,nlevels,p0,t0,gaircor,trans,ntrans)
c
c  Write results
c
      open(unit=2,file='linebyline.dat')
      if (case.eq.1) then
         do i=0,1500000
            vv=v1+i*dv
            if ((vv.ge.v1).and.(vv.le.v2)) then
               if (ntrans(i).eq.0) then
                  write(2,'(f9.3,f9.6,f13.6,f9.4)') 
     +             v1+i*dv,1.d0,0.d0,10000.d0/(v1+i*dv)
               else
                  write(2,'(f9.3,f9.6,f13.6,f9.4)') 
     +             v1+i*dv,trans(i)/ntrans(i),
     +                -log(trans(i)/ntrans(i)),10000.d0/(v1+i*dv)
               end if
            end if
         end do
      else if (case.eq.2) then
         do i=0,40000
            if (ntrans(i).ne.0) then
               taulast=0.d0
               rad=0.d0
               vv=v1+i*dv
               do j=nlevels,0,-1
                  tau=-log(trans(40000*j+i)/ntrans(40000*j+i))
                  dexp=exp(-taulast)-exp(-tau)
                  taulast=tau
                  tave=0.5d0*(t0(j)+t0(j+1))
                  rad=rad+planck(vv,tave)*dexp
               end do
               rad=rad+planck(vv,tsurf)*exp(-taulast)
               write(2,'(f9.3,e14.6,f12.7,f9.6,f9.4)')
     +                   vv,rad,taulast,exp(-taulast),10000.d0/vv
            end if
         end do
      end if
c
...
c***********************************************************************
      subroutine getlinebyline(case,nnn,mol,iso,uu,fmix,enrich,
     +              v1,v2,dv,nlevels,p0,t0,gaircor,trans,ntrans)
      implicit none
      character*6 x(38)/'h2o   ','co2   ','o3    ','n2o   ','co    ',
     +                  'ch4   ','o2    ','no    ','so2   ','no2   ',
     +                  'nh3   ','hno3  ','oh    ','hf    ','hcl   ',
     +                  'hbr   ','hi    ','clo   ','ocs   ','h2co  ',
     ...
     +                  'no+   ','hobr  ','c2h4  '/
      character*24 name
      integer mwt(130)/-1,18,20,19,19,21,20,
     +                -2,44,45,46,45,47,46,48,47,
     +     -3,48,50,50,49,49,-4,44,45,45,46,45,
     +     -5,28,29,30,29,31,30,-6,16,17,17,-7,32,34,33,
     +     -8,30,31,32,-9,64,66,-10,46,-11,17,18,
     +     -12,63,-13,17,19,18,-14,20,-15,36,38,
     +     -16,80,82,-17,128,-18,51,53,-19,60,62,61,61,62,
     +     -20,30,31,32,-21,52,54,-22,28,-23,27,28,28,
     +     -24,50,52,-25,34,-26,26,27,-27,30,
     +     -28,34,-29,66,-30,146,-31,34,36,35,
     +     -32,46,-33,33,-34,16,-35,97,99,
     +     -36,30,-37,96,98,-38,28,29/
      integer nnn,mol(19),iso(19),mm,isotope,i,j,k,ni,nline(19)
      integer ii,nlevels,case,jj,a0,amax,n0,ia,ij
      integer ntrans(0:1500000)
      real*4 xx(0:999999),yy,kk(0:999999),kabs(0:1000000)
      real*4 tau(0:1000000),ks,trans(0:1500000)
      real*8 uu(99,0:29),fmix(99),tt,t0(0:29),v0,v1,v2,enrich(19)
      real*8 v(19,29999),s(19,29999),gair(19,29999),gself(19,29999)
      real*8 e(19,29999),nalpha(19,29999),delta(19,29999),ddv
      real*8 molwt(99,9),h0,c0,k0,z1,z2,psatm(19),vgrid,dv,gi
      real*8 qratio(19),dplr(19),tref,qref,qt,wingcutoff,vline
      real*8 ss,alphad,alpha,p0(0:29),patm,alphadsqrtpi,gaircor
c
      tref=296.d0               ! reference temperature for the data
      h0=6.6260687d-27
      c0=2.99792458d+10
      k0=1.38065d-16
      wingcutoff=20000.d0        ! in units of max(alpha,alpha_D)
      vgrid=5.d-4               ! internal spacing for computation
      ddv=1000000.d0*vgrid       ! wavenumber window to do in one pass
c
c  Read in all the line data
c
      do ni=1,nnn
         i=6
         do while (x(mol(ni))(i:i).eq.' ')
            i=i-1
         end do
         name='linedata_'//x(mol(ni))(1:i)//'.dat'
         open(unit=1,file=name,status='old')
c
         ii=1
 10      read(1,*,end=19) v(ni,ii),s(ni,ii),gair(ni,ii),gself(ni,ii),
     +      e(ni,ii),nalpha(ni,ii),delta(ni,ii),mm,isotope
         if (gaircor.gt.0.d0) then
            gair(ni,ii)=gaircor
         else if (gaircor.lt.0.d0) then
            gair(ni,ii)=-gair(ni,ii)*gaircor
         end if
         if ((iso(ni).eq.isotope).and.(v(ni,ii).gt.v1-5.d0).and.
     +       (v(ni,ii).lt.v2+5.d0)) ii=ii+1
         goto 10
 19      nline(ni)=ii-1
         write(*,*) 'Read data: mol=',mol(ni),'iso=',iso(ni),
     +               '# lines=',nline(ni)
         close(1)
      end do
c
...
c
            if (case.eq.1) then
               patm=p0(jj)/1013.d0   ! pressure in atmospheres
            else
               patm=0.5d0*(p0(jj)+p0(jj+1))/1013.d0   ! pressure in atmospheres
            end if
            tt=0.5d0*(t0(jj)+t0(jj+1))
            z2=h0*c0*(1.d0/tt-1.d0/tref)/k0
            if (v0.eq.v1) write(*,'(a,i4,f9.4,f9.3)') 
     +                'level,p,T=',jj,1013.d0*patm,tt
c
c  Partition functions. Include isotopic enrichment since this just
c     muliplies the line strength
c
            do ni=1,nnn
               call BD_TIPS_2003(mol(ni),tt,iso(ni),gi,qt)
               call BD_TIPS_2003(mol(ni),tref,iso(ni),gi,qref)
c               call QofT(mol(ni),iso(ni),tt,qt)
c               call QofT(mol(ni),iso(ni),tref,qref)
               qratio(ni)=enrich(ni)*qref/qt
c
c  Factor in Doppler width. dplr = sqrt(2*R*T)/c
c     This is alpha_D in Liou's book, not alpha_D'
c     alpha_D' is the half-width and is sqrt(ln(2))*alpha_D
c
               dplr(ni)=sqrt(
     +            2.d0*8.314472d7*tt/molwt(mol(ni),iso(ni)))/c0
c
c  Partial pressure (in atm) of the molecule
c
               psatm(ni)=fmix(mol(ni))*patm
            end do
c
...
c
               do j=1,nline(ni)
                  if ((v(ni,j).ge.v0-20.d0).and.
     +                (v(ni,j).le.v0+ddv+20.d0)) then
c
c  The air-broadened pressure shift of the line center
c
                     vline=v(ni,j)+delta(ni,j)*psatm(ni)
c
c  Doppler width
c
                     alphad=vline*dplr(ni)
                     alphadsqrtpi=alphad*sqrt(3.14159265359d0)
c
c  Pressure broadening (by air)
c
                     alpha=(gair(ni,j)*(patm-psatm(ni))+
     +                 gself(ni,j)*psatm(ni))*(tref/tt)**nalpha(ni,j)
c
c  Line strength
c
                     z1=(1.d0-exp(-h0*c0*vline/(k0*tt)))/
     +                  (1.d0-exp(-h0*c0*vline/(k0*tref)))
                     ss=s(ni,j)*qratio(ni)*z1*exp(-z2*e(ni,j))
c
c  Compute Voigt profile
c
                     yy=alpha/alphad
                     if (yy.ge.1.d0) then
                        amax=min(nint(wingcutoff*alpha/vgrid),999999)
                     else
                        amax=min(nint(wingcutoff*alphad/vgrid),999999)
                     end if
                     a0=nint((vline-v0)/vgrid)
                     if ((a0+amax.ge.0).or.(a0-amax.le.1000000)) then
                        n0=amax+1
                        do i=0,amax
                           xx(i)=i*vgrid/alphad
                        end do
                        call HUMLIK2(n0,xx,yy,kk)
c
c  Sum up absorption coefficients
c
                        do i=1,amax
                           ks=ss*kk(i)/alphadsqrtpi
                           ia=a0+i
                           if ((ia.ge.0).and.(ia.le.1000000))
     +                           kabs(ia)=kabs(ia)+ks
                           ia=a0-i
                           if ((ia.ge.0).and.(ia.le.1000000))
     +                           kabs(ia)=kabs(ia)+ks
                        end do
                        if ((a0.ge.0).and.(a0.le.1000000))
     +                            kabs(a0)=kabs(a0)+
     +                            ss*kk(0)/alphadsqrtpi
                     end if
                  end if
               end do
c
c  Compute tau for this particular species and add to total
c
               do i=0,1000000
c                  tau(i)=tau(i)+kabs(i)*uu(mol(ni),jj)
                  tau(i)=tau(i)+2.d0*kabs(i)*uu(mol(ni),jj)
               end do
            end do
c
c  Average transmissivity over frequency to dilute to resolution of dv
c
            do i=0,999999
               ii=nint((v0+i*vgrid-v1)/dv)
               if (tau(i).gt.0.d0) then
                  ij=40000*jj+ii
                  trans(ij)=trans(ij)+exp(-tau(i))
                  ntrans(ij)=ntrans(ij)+1
c                  write(99,*) i,v0+i*vgrid,
c     +                          tau(i)*2.687d19/uu(mol(1),jj)
c                  if (mod(i,10).eq.0) write(98,*) i,exp(-tau(i))
               end if
            end do
          end do
          v0=v0+ddv
