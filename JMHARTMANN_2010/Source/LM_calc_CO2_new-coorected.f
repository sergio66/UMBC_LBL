...
C Min and Max wavenumbers and step of the calculation (user supplied)
      sgminR=4750.d0   ! Min wavenumber (cm-1)
      sgmaxR=5200.d0   ! Max wavenumber (cm-1)
      dsg=0.1d0        ! wavenumber step (cm-1)
...
C
	Ptot=54.D0       ! Total pressure (Atm)
	Temp=218.15d0    ! Tmperature (K)
	xCO2=1.d-2       ! CO2 molefraction
	xH2O=0.d-2       ! H2O molefraction
c Choose full LM calclation (TRUE) or not (FALSE) (user supplied)
      MixFull=.TRUE.
C --------- 
C Calculate absorptio coefficient
C

c this is to dump out input params so we can test against Total_program_win.f
c      print *,SgMinR,SgMaxR,DSg,Ptot,Ptot*xCO2,Ptot*xH2O,Temp,1.000
c      stop
	 call CompAbs(Temp,Ptot,xCO2,xH2O,SgMinR,SgMaxR,DSg
     &      ,AbsV,AbsY,AbsW,MixFull,Nsig)
C 
C print results in File
      open (unit=55,file='./LM_TEST.dat',status='unknown')
      do isig=1,Nsig
...
C Open File Giving Information on Bands, Read, and Select
      nBand=0
      Open(Unit=iFile,File='../Data_new/BandInfo.dat',Status='Old')
1     Read(iFile,1000,End=100)isotR,c1,lfR,C2,c3,liR,c4,c5,
     &                         Stot,SgMin,SgMax,JmxP,JmxQ,JmxR
c      print *, isotR,' ',c1,' ',lfR,' ',C2,' ',c3,' ',liR,' ',c4,' ',c5,' ',
c     &                         Stot,' ',SgMin,' ',SgMax,' ',JmxP,' ',JmxQ,' ',JmxR
1000  Format(I1,2(A2,i1,A2),a2,E12.4,2(1x,f12.6),8x,3I4)
c      stop

c so the first line is
c  1  00   0  11 00   0  01      9.153000000000000E-017    1416.43051000000    3313.83051000000    124   -1    124
C
...
C     
      ...
      real*8 HWT0AIR,BHWAIR,HWT0H2O,BHWH2O,SHFT0
...
C     Call the Subroutine for calculation of the Voigt function
                  YY=HWT(iLine)*Cte
                  XX=(Sig(iLine,iBand)+SHFT(iLine)*Ptot-SigC)*Cte
...
C     Shift to Lorentz profile outside +/- rdmult*GamD
     ...
     &        (u_pi)*(HWT(iLine))/
     & (HWT(iLine)**2+(SigC-Sig(iLine,iBand)-SHFT(iLine)*Ptot)**2)
...
C     
      ...
      real*8 HWT0AIR,BHWAIR,HWT0H2O,BHWH2O,SHFT0
	real*8 Sig,Dipo0,E,PopuT0,S,PFCO2
      real*8 HWT,PopuT,YT,DipoT,SHFT
...
C*********************************************************************
      ...
      real*8 Sig,Dipo0,E,HWT,PopuT,DipoT,shft
