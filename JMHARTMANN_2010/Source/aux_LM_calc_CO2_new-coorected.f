...
C
C
cccccccccc      print*, 'Reading the W files...'
...
C     
      ...
      integer*4 nSig,iSig,iBand,iLine,ji,jf,nSig0
...
C     
      nsig0 = nSig       !!! this is the input we send in
C     Check that Arrays for Results are Large Enough, Initialize
      nSig=dInt( ((SigMax-SigMIn)/DSig)+0.5d0 )+1
      If ( nSig .GT. nSigmx) Then
         print *,'arrays are SMALLER than number of points req'
         Write(*,1000)nSig,nSigMx
         Stop
         End If
      If ( nSig .ne. nSig0) Then
         print *,'SigMin,SigMax,dSig = ',SigMIn,SigMax,DSig
         print *,'nSig = dInt( ((SigMax-SigMIn)/DSig)+0.5d0 )+1 = ',nSig
         print *,'computed length (nSig) and input length (nSig0) are different'
         print *,'nSig,nSig0 = ',nSig,nSig0
         IF (abs(nSig-nSig0) .LE. 1) THEN
           print *,'Close enough (differ by one)'
           nSig = nsig0
         ELSE
           print *,'nSig and nSig0 differ by more than one, STOPPING'
           Write(*,1000)nSig,nSig0
           Stop
           END IF
         End If
c      print *,'hahahaha ',SigMin,SigMax,DSig,nsig,nsig0,nsigmx
c      stop

 1000 Format(1x,'************ PROBLEM !!!! ******************',
     &     /,1x,'>>> aux_LM_calc_CO2_new-corrected',
