function [outwave,out_array] = run8co2_weakbackgnd(gasID,fmin,fmax,profname,topts,toptsRMV)
...
%%% optimized for 4 um branch (so use default LVF = 'V' instead of 'G')
%%% BIG DIFFERENCES in DEFAULT : (from run7co2 to this code)
%%% a) only do weak lines (ie take away the strong bands such as 2350,2320 etc)
%%% b) xfar = 25.0 cm-1 instead of 250.0 cm-1

%%% c) birn = 'c' so all lines get cousin tacked onto them! (insetad of 'b')
%%% d) IO =   '0' for NO Linemixing (instead of '1')
%%% d) LVF =  'G' for NO Linemixing (instead of 'F')

% Does NOT do any line mixing : simply does Voigt for ALL lines except
%     those specified in toptsRMV which has fields
%       iL = toptsRMV.iL(iii);        %%lower state quantum number
%       iU = toptsRMV.iU(iii);        %%upper state quantum number
%       iS = toptsRMV.iISO(iii);      %%isotope
%       iB = toptsRMV.iKCBand(iii);   %%kCARTABAND ID
% ie same as run7co2 except 
%   a) always requires iLineMixLoop = -1
%   b) removelines is modified to remove ALL lines from a band
...
%
%char      IO             '1' for first order line mixing          '0'
...
% HISTORY : 
% 22) Does NNOT do any line mixing : simply does Voigt for ALL lines except
%     those specified in toptsRMV
...
%      /home/sergio/KCARTA/NONLTE/M_Files_for_kcarta_NLTE_LBL_runs/USUALLAYERS/cluster_loop_background.m
...
%        path(path,'/salsify/users/sergio/KCARTA/SPECTRA/Q_deltpi');
%        path(path,'/salsify/users/sergio/KCARTA/SPECTRA/Q_sigpi');
%        path(path,'/salsify/users/sergio/KCARTA/SPECTRA/PR_sigsig');
%        path(path,'/salsify/users/sergio/KCARTA/SPECTRA/PR_deltdelt');
%        path(path,'/salsify/users/sergio/KCARTA/SPECTRA/PR_pipi');
%        path(path,'/salsify/users/sergio/KCARTA/SPECTRA/PR_sigpi');
...
global quiet p2311_21_jmax p2350_jmax r2350_jmax pr2351_jmax stretch 
...
if (gasID ~= 2)
  error('this is a specialised code for CO2. use run7 for other gases');
  end
...
% note L1,L0; V1,V0 are different while F1,F0 are the same
%
%char     birn            'b','B' for include birnbaum              'c'
%                         'c','C' for include cousin
%                         'n','N' for no birnbaum/cousin 
%                         (always off for Q-sigpi,Q-deltpi)
%
...
xnear         = 1.0;
xmed          = 2.0;
xfar          = 25.0;          %%%% <---- note this, instead of 250.0 *******
nbox          = 5;
strength_far  = 0.0;
strength_near = 0.0;
...
if topts.RemoveSomeBands < 0
  error('ooh cannot do this!');
  fprintf(1,'\n finding start/stop wavenumbers of bands \n');
  [hh,ll]=band_end_find(gasID,homepath,band,vers,strengthM,fname);
...
fprintf(1,'initial number of background lines = %3i \n',lineORIG.linct);

lineORIG0 = lineORIG;
lineORIG  = translate2oldHITparams(lineORIG);
if topts.RemoveSomeBands > 0
  disp('ooops .... gotta do this NLTE thingy!!!')
  %%start reading info from toptsRMV
  for iii = 1 : length(toptsRMV.iL)
    clf; semilogy(lineORIG.wnum,lineORIG.stren,'.-'); hold on

    iAll = 1 : length(lineORIG.iso);
    iL = toptsRMV.iL(iii);        %%lower state quantum number
    iU = toptsRMV.iU(iii);        %%upper state quantum number
    iS = toptsRMV.iISO(iii);      %%isotope
    iB = toptsRMV.iKCBand(iii);   %%kCARTABAND ID
    i1 = find((lineORIG.ilsgq==iL) & (lineORIG.iusgq==iU) & ...
              (lineORIG.iso==iS));
    i10 = i1;	      
    ii = setdiff(iAll,i1);

    semilogy(lineORIG.wnum(i1),lineORIG.stren(i1),'r.-'); hold off; grid;
    fprintf(1,'iL,iU,iS,iBand = %3i %3i %3i %5i \n',iL,iU,iS,iB);
    fprintf(1,'  removed %6i lines new length = %6i \n  \n',length(i10),length(ii));
      pause(0.1);
...
semilogy(lineORIG.wnum,lineORIG.stren,'.-'); hold off; grid;
...
%plot(line.wnum,line.tsp); title('Lines2use'); pause
  ...
  for  ii=1:noloops    %OUTER LOOP OVER WIDE MESH 
    ...
    for jj=MinLayer:Step:MaxLayer 
      ...
      if very_near.linct > 0 
        ...
        s0       = very_near.stren; 
        brd      = broad_weakco2(p,ps,1.0,for_brd,self_brd,pwr,tempr,gasID); 
...
if ((num_band > 0) & (noloops > 0))
  error('huh??? cannot do linemixing in THIS code!!!!');
  end
