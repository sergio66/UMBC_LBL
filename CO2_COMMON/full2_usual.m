function [fullmix4]=full2_usual(freqq,f,W_plus,jq,w_selfq,w_forq,...
...
%       using FORTRAN 
%       It reorders the eigenfreqs
%       It is used for all PQR branches, except dela-pi (668,740,2093 and 2322)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
bwah     = +1;       %%%%%%%DO HAVE THIS SET TO +1 to order things correctly
plotbwah = -1;
...
if (ratio >= 1e-2) 
  ...
  if (plotbwah > 0)
    ...
    subplot(211);semilogy(abs(diagL),abs(arg1.*arg2'),freqq_shift,strenqt);
    subplot(212);semilogy(l1,abs(arg1.*arg2'),l1,strenqt);
    title(num2str(band)); pause(1)
    end
  ...
  [gg,hh]=sort(strenqt); maxx=floor(0.33*length(strenqt));
  indx=hh(maxx:length(strenqt));
  indx1=hh(1:maxx-1);

  %be more careful .. use only 2/3 lowesr j lines for the mixing calcs
  ...
  w_tot=(pressure_self*w_selfq(indx)+pressure_for*w_forq(indx))/pressure_ref; 
