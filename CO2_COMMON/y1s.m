%% this is from cp -a /home/sergio/KCARTA/NONLTE/M_Files_for_kcarta_NLTE_LBL_runs/USUALLAYERS/y1s.m y1s_kcarta_nlte.m

%% >>> should be same as what is in /home/sergio/SPECTRA/CO2_COMMON/y1s.m <<<
%% and there when it runs, you need to set
%%   iNLTE_Print = +1
...
if iNLTE_Print > 0
  ...
  outfiledir = ['/asl/data/kcarta_sergio/KCDATA/NLTE/AIRSCO2/LINEMIX_H' hitstr '_' num2str(co2ppm) '/PQR/'];
  outfiledir = ['/asl/data/kcarta_sergio/KCDATA/NLTE/AIRSCO2/TEST0_LINEMIX_H' hitstr '_' num2str(co2ppm) '/PQR/'];  

  outfile = [outfiledir num2str(round(stuff.temperature)) 'K_band' num2str(stuff.band)];
  ...
  if exist(outfile)
    fprintf(1,'   WARNING %s already exists! \n',outfile)
    %error('check out where you are writing files to!!!')
  end
