...
%%%% from the JJOHNS fits we need to modify the foreign params by 1.14
jjohns_doc = [1.0 1.00 1.14 1.14];     %%%from JJOHNS spectra 2,3
jjohns_doc = [1.0 1.00 1.74 1.74];     %%%from JJOHNS spectra 1,4
dur = dur.*jjohns_doc;
dur=dur*1e-3;
duration_self = dur(1);
...
elseif  ((band == 2310)|(band == 2311)  |  ...                  %deltdelt
  ...
  %%%% from the JJOHNS fits we need to modify the foreign params by 0.982
  jjohns_beta = [1.0 1.000 0.982 0.982];       %%%from JJOHNS spectra 2,3
  jjohns_beta = [1.0 1.000 0.940 0.940];       %%%from JJOHNS spectra 1,4
  besp = besp.*jjohns_beta;
