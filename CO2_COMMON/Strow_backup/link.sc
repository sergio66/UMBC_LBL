:

### July 1, 2000
### have removed the foolowing links as the code is not used
###   ln -s ../CO2_COMMON/full_mix4.m          full_mix4.m
###   ln -s ../CO2_COMMON/klormix.m            klormix.m
###   ln -s ../CO2_COMMON/klormixORIG.m        klormixORIG.m
###   ln -s ../CO2_COMMON/voigtmix.m           voigtmix.m
###   ln -s ../CO2_COMMON/cousin.m             cousin.m
###   ln -s ../CO2_COMMON/quicklorentz.m       quicklorentz.m

#################### 15 um bands

echo 'cd ../Q_sigpi'
cd ../Q_sigpi
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m     co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender15um.m
rm IncludeMaxer.m
rm full2_usual.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/blender15um.m        blender15um.m
ln -s ../CO2_COMMON/removeneg.m          removeneg.m    
ln -s ../CO2_COMMON/co2_param.m          co2_param.m
ln -s ../CO2_COMMON/doratio.m            doratio.m
ln -s ../CO2_COMMON/efit.m               efit.m
ln -s ../CO2_COMMON/full2.m              full2.m
ln -s ../CO2_COMMON/full2_usual.m        full2_usual.m
ln -s ../CO2_COMMON/orderer.m            orderer.m
ln -s ../CO2_COMMON/qqttiippss.m         qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m  smooth_full_ratio.m 
ln -s ../CO2_COMMON/tempratio15um.m      tempratio15um.m
ln -s ../CO2_COMMON/trans_pop.m          trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m          voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m      voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                y1s.m
ln -s ../IncludeMaxer.m                  IncludeMaxer.m 

echo 'cd ../PR_sigpi'
cd ../PR_sigpi
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m     co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender15um.m
rm IncludeMaxer.m
rm full2_usual.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/blender15um.m        blender15um.m
ln -s ../CO2_COMMON/removeneg.m          removeneg.m    
ln -s ../CO2_COMMON/co2_param.m          co2_param.m
ln -s ../CO2_COMMON/doratio.m            doratio.m
ln -s ../CO2_COMMON/efit.m               efit.m
ln -s ../CO2_COMMON/full2.m              full2.m
ln -s ../CO2_COMMON/full2_usual.m        full2_usual.m
ln -s ../CO2_COMMON/orderer.m            orderer.m
ln -s ../CO2_COMMON/qqttiippss.m         qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m  smooth_full_ratio.m 
ln -s ../CO2_COMMON/tempratio15um.m      tempratio15um.m
ln -s ../CO2_COMMON/trans_pop.m          trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m          voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m      voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                y1s.m
ln -s ../IncludeMaxer.m                  IncludeMaxer.m 

echo 'cd ../Q_deltpi'
cd ../Q_deltpi
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m  co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender15um.m
rm IncludeMaxer.m
rm full2_deltpi.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/blender15um.m        blender15um.m
ln -s ../CO2_COMMON/removeneg.m          removeneg.m    
ln -s ../CO2_COMMON/co2_param.m          co2_param.m
ln -s ../CO2_COMMON/doratio.m            doratio.m
ln -s ../CO2_COMMON/efit.m               efit.m
ln -s ../CO2_COMMON/full2.m              full2.m
ln -s ../CO2_COMMON/full2_deltpi.m       full2_deltpi.m
ln -s ../CO2_COMMON/orderer.m            orderer.m
ln -s ../CO2_COMMON/qqttiippss.m         qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m  smooth_full_ratio.m 
ln -s ../CO2_COMMON/tempratio15um.m      tempratio15um.m
ln -s ../CO2_COMMON/trans_pop.m          trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m          voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m      voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                y1s.m
ln -s ../IncludeMaxer.m                  IncludeMaxer.m 

echo 'cd ../PR_deltpi'
cd ../PR_deltpi
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m  co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender15um.m
rm IncludeMaxer.m
rm full2_deltpi.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/blender15um.m        blender15um.m
ln -s ../CO2_COMMON/removeneg.m          removeneg.m    
ln -s ../CO2_COMMON/co2_param.m          co2_param.m
ln -s ../CO2_COMMON/doratio.m            doratio.m
ln -s ../CO2_COMMON/efit.m               efit.m
ln -s ../CO2_COMMON/full2.m              full2.m
ln -s ../CO2_COMMON/full2_deltpi.m       full2_deltpi.m
ln -s ../CO2_COMMON/orderer.m            orderer.m
ln -s ../CO2_COMMON/qqttiippss.m         qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m  smooth_full_ratio.m 
ln -s ../CO2_COMMON/tempratio15um.m      tempratio15um.m
ln -s ../CO2_COMMON/trans_pop.m          trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m          voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m      voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                y1s.m
ln -s ../IncludeMaxer.m                  IncludeMaxer.m 

#################### 4 um bands

echo 'cd ../PR_deltdelt'
cd ../PR_deltdelt
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m   co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender4um.m
rm IncludeMaxer.m
rm full2_usual.m
rm co2_param_JOHNS2002.m ral_jjohn_blend3.m ral_jjohn_blend.m small_leap.m
rm findregions.m
rm fudger_and_threeregions.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/fudger_and_threeregions.m fudger_and_threeregions.m
ln -s ../CO2_COMMON/blender4um.m         blender4um.m
ln -s ../CO2_COMMON/removeneg.m          removeneg.m    
ln -s ../CO2_COMMON/tempratio4um.m       tempratio4um.m
ln -s ../CO2_COMMON/co2_param.m          co2_param.m
ln -s ../CO2_COMMON/doratio.m            doratio.m
ln -s ../CO2_COMMON/efit.m               efit.m
ln -s ../CO2_COMMON/full2.m              full2.m
ln -s ../CO2_COMMON/full2_usual.m        full2_usual.m
ln -s ../CO2_COMMON/orderer.m            orderer.m
ln -s ../CO2_COMMON/qqttiippss.m         qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m  smooth_full_ratio.m 
ln -s ../CO2_COMMON/trans_pop.m          trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m          voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m      voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                y1s.m
ln -s ../IncludeMaxer.m                  IncludeMaxer.m 
ln -s ../CO2_COMMON/co2_param_JOHNS2002.m  co2_param_JOHNS2002.m 
ln -s ../CO2_COMMON/small_leap.m           small_leap.m
ln -s ../CO2_COMMON/ral_jjohn_blend3.m     ral_jjohn_blend3.m
ln -s ../CO2_COMMON/ral_jjohn_blend.m      ral_jjohn_blend.m
ln -s ../CO2_COMMON/find_regions.m         find_regions.m
 
echo 'cd ../PR_pipi'
cd ../PR_pipi
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m     co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender4um.m
rm IncludeMaxer.m
rm full2_usual.m
rm full2_deltpi.m
rm co2_param_JOHNS2002.m ral_jjohn_blend3.m ral_jjohn_blend.m small_leap.m
rm find_regions.m
rm fudger_and_threeregions.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/fudger_and_threeregions.m fudger_and_threeregions.m
ln -s ../CO2_COMMON/blender4um.m         blender4um.m
ln -s ../CO2_COMMON/removeneg.m          removeneg.m    
ln -s ../CO2_COMMON/tempratio4um.m       tempratio4um.m
ln -s ../CO2_COMMON/co2_param.m          co2_param.m
ln -s ../CO2_COMMON/doratio.m            doratio.m
ln -s ../CO2_COMMON/efit.m               efit.m
ln -s ../CO2_COMMON/full2.m              full2.m
ln -s ../CO2_COMMON/full2_usual.m        full2_usual.m
ln -s ../CO2_COMMON/full2_deltpi.m       full2_deltpi.m
ln -s ../CO2_COMMON/orderer.m            orderer.m
ln -s ../CO2_COMMON/qqttiippss.m         qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m  smooth_full_ratio.m 
ln -s ../CO2_COMMON/trans_pop.m          trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m          voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m      voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                y1s.m
ln -s ../IncludeMaxer.m                  IncludeMaxer.m 
ln -s ../CO2_COMMON/co2_param_JOHNS2002.m  co2_param_JOHNS2002.m 
ln -s ../CO2_COMMON/small_leap.m           small_leap.m
ln -s ../CO2_COMMON/ral_jjohn_blend3.m     ral_jjohn_blend3.m
ln -s ../CO2_COMMON/ral_jjohn_blend.m      ral_jjohn_blend.m
ln -s ../CO2_COMMON/find_regions.m         find_regions.m

echo 'cd ../PR_sigsig'
cd ../PR_sigsig
rm full2.m          orderer.m        voigtmixRatio.m
rm trans_pop.m      tempratio*.m     co2_param.m
rm efit.m           voigtmix2.m      y1s.m
rm smooth_full_ratio.m  doratio.m    qqttiippss.m
rm removeneg.m          blender4um.m
rm IncludeMaxer.m
rm full2_usual.m
rm co2_param_JOHNS2002.m ral_jjohn_blend3.m ral_jjohn_blend.m small_leap.m
rm find_regions.m
rm fudger_and_threeregions.m
rm efit1.m
ln -s ../CO2_COMMON/efit1.m              efit1.m
ln -s ../CO2_COMMON/fudger_and_threeregions.m fudger_and_threeregions.m
ln -s ../CO2_COMMON/blender4um.m           blender4um.m
ln -s ../CO2_COMMON/removeneg.m            removeneg.m    
ln -s ../CO2_COMMON/tempratio4um.m         tempratio4um.m
ln -s ../CO2_COMMON/co2_param.m            co2_param.m
ln -s ../CO2_COMMON/doratio.m              doratio.m
ln -s ../CO2_COMMON/efit.m                 efit.m
ln -s ../CO2_COMMON/full2.m                full2.m
ln -s ../CO2_COMMON/full2_usual.m          full2_usual.m
ln -s ../CO2_COMMON/orderer.m              orderer.m
ln -s ../CO2_COMMON/qqttiippss.m           qqttiippss.m
ln -s ../CO2_COMMON/smooth_full_ratio.m    smooth_full_ratio.m 
ln -s ../CO2_COMMON/trans_pop.m            trans_pop.m
ln -s ../CO2_COMMON/voigtmix2.m            voigtmix2.m
ln -s ../CO2_COMMON/voigtmixRatio.m        voigtmixRatio.m
ln -s ../CO2_COMMON/y1s.m                  y1s.m
ln -s ../IncludeMaxer.m                    IncludeMaxer.m 
ln -s ../CO2_COMMON/co2_param_JOHNS2002.m  co2_param_JOHNS2002.m 
ln -s ../CO2_COMMON/small_leap.m           small_leap.m
ln -s ../CO2_COMMON/ral_jjohn_blend3.m     ral_jjohn_blend3.m
ln -s ../CO2_COMMON/ral_jjohn_blend.m      ral_jjohn_blend.m
ln -s ../CO2_COMMON/find_regions.m         find_regions.m

echo 'doing the 4um drivers'
echo 'cd ../PR_sigsig'
cd ../PR_sigsig
rm driver*.m
ln -s ../CO2_COMMON/MAIN4umDRIVERS/driver4um.m  driver4um.m

echo 'cd ../PR_pipi'
cd ../PR_pipi
rm driver4um.m
ln -s ../CO2_COMMON/MAIN4umDRIVERS/driver4um.m driver4um.m

echo 'cd ../PR_deltdelt'
cd ../PR_deltdelt
rm driver4um.m
ln -s ../CO2_COMMON/MAIN4umDRIVERS/driver4um.m driver4um.m

echo 'doing the 15um drivers'
echo 'cd ../Q_sigpi'
cd ../Q_sigpi
rm driver*
ln -s ../CO2_COMMON/MAIN15umDRIVERS/driver15um.m    driver15um.m

echo 'cd ../PR_sigpi'
cd ../PR_sigpi
rm driver*
ln -s ../CO2_COMMON/MAIN15umDRIVERS/driver15um.m       driver15um.m
ln -s ../CO2_COMMON/MAIN15umDRIVERS/driver15umPR.m     driver15umPR.m
rm voigt_mix_simple2.m
ln -s ../CO2_COMMON/voigt_mix_simple2.m   voigt_mix_simple2.m

echo 'cd ../Q_deltpi'
cd ../Q_deltpi
rm driver*
ln -s ../CO2_COMMON/MAIN15umDRIVERS/driver15um.m driver15um.m

echo 'cd ../PR_deltpi'
cd ../PR_deltpi
rm driver*
ln -s ../CO2_COMMON/MAIN15umDRIVERS/driver15um.m       driver15um.m
ln -s ../CO2_COMMON/MAIN15umDRIVERS/driver15umPR.m     driver15umPR.m
rm voigt_mix_simple2.m
ln -s ../CO2_COMMON/voigt_mix_simple2.m   voigt_mix_simple2.m

echo 'cleaning up the subdirs ... '
cd ../Q_sigpi;     rm *~; 
cd ../PR_sigpi;    rm *~; 
cd ../Q_deltpi;    rm *~; 
cd ../PR_deltpi;   rm *~; 
cd ../PR_deltdelt; rm *~; 
cd ../PR_sigsig;   rm *~; 
cd ../PR_pipi;     rm *~; 

cd ../CO2_COMMON;      rm *~;
  cd MAIN15umDRIVERS;    rm *~;
  cd ../MAIN4umDRIVERS;  rm *~;
cd ../../CO2_COMMON;     rm *~;

cd ..;  rm *~;
cd CO2_COMMON

echo 'done!!'