...
%this is to do lorentz
%ylor=zeros(size(outwave));
%w_tot=(pself*w_selfr+(ptotal-pself)*w_forr)/stuff.pressure_ref; 
%for ii=1:length(jr)
%  ylor=ylor + strenrt(ii)*lorentz(outwave,freqr(ii),temperature,46,w_tot(ii));
%  end
%ylor=ylor*6.026e26*layeramt;
