function [junk,outwave,out_array] = run8_parallel(gasID,fmin,fmax,profname,topts)
...
allowedparams = [{'ffin'},     {'fmed'},         {'fcor'},     {'fstep'}, ...
                 ...
                 {'which_isotope'} {'O2O3N2continuumVers'}];
...
if (number_of_lines > 0)
  ...
  %%for  ii=1:-1    %OUTER LOOP OVER WIDE MESH
    ...
    fprintf(1,'gasid %3i mesh %3i start stop freq = %8.5f %8.5f',gasID,ii,f3,f4);
  ...
  if (length(outwave) <= MaxLen)          %can do it in one big chunk!
    for jj=MinLayer:Step:MaxLayer %INNER LOOP 1..100 = bottom -> top
      ...
      switch O2O3N2continuumVers
        case 1
          scum = calconV1(gasID,length(outwave),outwave,1,length(press),...
                  temperature,press,partpress,GasAmt,CKD,jj);
        case 3	  
          scum = calconV3(gasID,length(outwave),outwave,1,length(press),...
                  temperature,press,partpress,GasAmt,CKD,jj);
        case 4
          scum = calconV4(gasID,length(outwave),outwave,1,length(press),...
                  temperature,press,partpress,GasAmt,CKD,jj);
       otherwise
    ...
    for jj=MinLayer:Step:MaxLayer %LOOP 1..100 = bottom -> top
      ...
      for kk=1:number             %LOOP OVER FREQUENCY INTERVALS
        ...
        switch O2O3N2continuumVers
          case 1
            scum = calconV1(gasID,length(index),outwave(index),1,...
              length(press),temperature,press,partpress,GasAmt,CKD,jj);
          case 3
            scum = calconV3(gasID,length(index),outwave(index),1,...
              length(press),temperature,press,partpress,GasAmt,CKD,jj);
          case 4
            scum = calconV4(gasID,length(index),outwave(index),1,...
              length(press),temperature,press,partpress,GasAmt,CKD,jj);
          otherwise
            error('only have o2/n2/o3 continuum Vers 1,3,4');
      end
