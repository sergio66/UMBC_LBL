...
c pretty much SAME as doVmixg.f except that it has TWO more arguments for the
c extra exponential chi function ....
...
c c0,cpsan       = params for the gaussian chi function 
      ...
      integer zp,vp,v0p,Tp,tau2p,w_totp,klmp,kvmp,bsmp,
     $        pforp,pselfp,birnp,nifp,massp,strenqtp,ymixp,
     $        c0p,cspanp
      real*8 raV(MaxLen),raZ(MaxLen),rTp,raV0(MaxPQR),rtau2p,
     $          rpforp,rpselfp,rkvmp,rklmp,rbsmp,rbirnp,ra_Wtot(MaxPQR),
     $          raStrenqt(MaxPQR),raYmix(MaxPQR),rnifp,rmassp,
     $          rc0p,rcspanp
...
c copy right hand arguments to local arrays or variables        
      ...
      call mxCopyPtrToReal8(c0p,rc0p,1)
      call mxCopyPtrToReal8(cspanp,rcspanp,1)
...
c      print *,'gateway lenf, no_lines are ',int(nnin),int(llin)
c      print *,'gateway c0,span = ',rc0p,rcspanp
      call doVmix2(raZ,raV,raV0,ra_Wtot,rTp,rtau2p,rpforp,rpselfp,
     $    rmassp,raStrenqt,raYmix,rkvmp,rklmp,rbsmp,rbirnp,rnifp,
     $    rc0p,rcspanp,int(nnin),int(llin))
