...
c brd  = broadening
      ...
      DOUBLE PRECISION wr(kMaxPts*5+4),v(kMaxPts*5+4),v0,Temp,m,brd
      DOUBLE PRECISION wi(kMaxPts*5+4)
      integer n_in
...
c local variables
       ...
       integer ii
       complex U,T,VTP(kMaxPts*5+4),VTM(kMaxPts*5+4)
...
c local variables
      ...
      IF (n_in .gt. kMaxPts*5+4) THEN
        write(kStdErr,*)'in vhh2RI.f, n_in .gt. kMaxPts*5+4',n_in,kMaxPts*5+4
        Call DoStop
        END IF
