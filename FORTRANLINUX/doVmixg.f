...
c check for proper number of arguments
      if (nrhs .ne. 15) then
        call mexErrMsgTxt('15 input args required')
        endif
      if (nlhs .ne. 1) then
...
c   do the actual computations in a subroutine
      call doVmix(raZ,raV,raV0,ra_Wtot,rTp,rtau2p,rpforp,rpselfp,rmassp,
     $    raStrenqt,raYmix,rkvmp,rklmp,rbsmp,rbirnp,rnifp,
     $    int(nnin),int(llin))
