function [outwave,out_array] = run8_g27(gasID,fmin,fmax,profname,topts)
...
if gasID ~= 27
  %% there are two isotopes in the H12 line database, but only one isotope
  %% Global_Data_HITRAN2008/molparam.txt
  error('sorry this is for gas27 only!!!!')
end
...
mass_iso = [30 30];   disp('----------->>>>> for g27, updating mass_iso')
  ...
  %%for  ii=1:-1    %OUTER LOOP OVER WIDE MESH
    ...
    for jj=MinLayer:Step:MaxLayer 
      ...
      if far_wing.linct > 0
        ...
        elseif (length(coarse_freq) > 3)  
          ddddd = isnan(coarse_freq); sum(ddddd);
          ddddd = isnan(z); sum(ddddd);
          if sum(ddddd) > 0
             figure(1); plot(freq,strength,'ro-');
             figure(2); plot(coarse_freq,'o-');
             figure(3); plot(output_freq,'ro-');
             disp('ooops problems in run8_g27')
           end
