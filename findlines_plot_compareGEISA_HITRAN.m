...
if mod(HITRANyear,4) ~= 0
  error('HITRANyear should be 2002,2004,....')
end  

[iYes1,line1] = findlines_plot(wv1,wv2,gid,GEISAyear,-1);
disp(' ')
[iYes2,line2] = findlines_plot(wv1,wv2,gid,HITRANyear);

figure(1); clf; semilogy(line1.wnum,line1.stren,'b.'); title(['GEISA ' num2str(GEISAyear)]); 
  fprintf(1,'GEISA  %4i has %6i lines \n',GEISAyear,length(line1.wnum))
ax1 = axis;
figure(2); clf; semilogy(line2.wnum,line2.stren,'r.'); title(['HITRAN ' num2str(HITRANyear)]); 
  fprintf(1,'HITRAN %4i has %6i lines \n',HITRANyear,length(line2.wnum))
ax2 = axis;
...
figure(1); plot(line1.wnum(oo1(wah)),line2.stren(oo2(oo21(wah)))./line1.stren(oo1(wah))); grid
    title(['STREN iso=1 ' num2str(HITRANyear) '/' num2str(GEISAyear)]); 
figure(2); semilogy(line2.wnum(oo2(oo21(wah))),line2.stren(oo2(oo21(wah))),'rx',line1.wnum(oo1(wah)),line1.stren(oo1(wah)),'b.'); grid
    title(['STREN iso=1 (r)H' num2str(HITRANyear) '(b)G' num2str(GEISAyear)]); 
disp('ret')
...
if length(I) > 0
  ...
  semilogy(line2.wnum,line2.stren,'r.',line1.wnum,line1.stren,'bo'); grid
    title(['STREN (b)G' num2str(GEISAyear) ' (r)H' num2str(HITRANyear)]);
  figure(4); clf
    ...
    oo1 = find(line1.iso == 3);     oo2 = find(line2.iso == 3);      
      ...
      hold off
    str1a = ['H' num2str(HITRANyear) ' iso1'];     str1b = ['G' num2str(GEISAyear) ' iso1'];
    str2a = ['H' num2str(HITRANyear) ' iso2'];     str2b = ['G' num2str(GEISAyear) ' iso2'];
    str3a = ['H' num2str(HITRANyear) ' iso3'];     str3b = ['G' num2str(GEISAyear) ' iso3'];    
  ...
  elseif iDo > 0
    ...
    title(['STREN G' num2str(GEISAyear) '-H' num2str(HITRANyear)]); 
    grid on
  elseif iDo == 0
    figure(4); clf; plot(line1.wnum(i1),100*(line2.stren(i2) - line1.stren(i1))./line2.stren(i2),'k+'); grid
    title(['STREN percent diff ' num2str(HITRANyear) '-' num2str(GEISAyear)]); 
    grid on
  end

  figure(5);
  semilogy(line2.wnum,line2.abroad,'r.',line1.wnum,line1.abroad,'bo'); grid
    title(['ABROAD (b)G' num2str(GEISAyear) ' (r)H' num2str(HITRANyear)]); 
  ...
  elseif iDo > 0
    ...
    title(['ABROAD ' num2str(HITRANyear) '-' num2str(GEISAyear)]); 
    grid on
  end

  figure(7);
  semilogy(line2.wnum,line2.sbroad,'r.',line1.wnum,line1.sbroad,'bo'); grid
    title(['SBROAD (b)G' num2str(GEISAyear) ' (r)H' num2str(HITRANyear)]); 
  ...
  elseif iDo > 0
    ...
    title(['SBROAD ' num2str(HITRANyear) '-' num2str(GEISAyear)]); 
    grid on
  end
