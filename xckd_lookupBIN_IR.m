...
theprofname = 'IPFILES/watercont300';  %% basically [1  1.00 0.01 300.0 1e-6]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fout_s = [foutdir 'xCKDSelf' num2str(CKD) '.bin'];
...
fout_s = [foutdir 'xCKDFor' num2str(CKD) '.bin'];
fid = fopen(fout_s,'w','ieee-le');
...
if CKD == 25
  ...
  figure(3); 
  semilogy(fr,ks,'b',lala(:,1),lala(:,2),'c',...
           fr,kf,'r',lala(:,1),lala(:,3),'m')
  hl = legend('ks','self fileunit IPU','kf','forn fileunit IPU');
  set(hl,'fontsize',10)

  figure(2); 
  baaS = interp1(lala(:,1),lala(:,2),fr);
  baaF = interp1(lala(:,1),lala(:,3),fr);
  plot(fr,ks./baaS,'b',fr,kf./baaF,'r')
  hl = legend('ks/baaS','kf/baaF');
  set(hl,'fontsize',10)
end
