...
!
! symbol table
!  calcon_loc_mtckd_43_wrap.o
!  calconwater_loc_ckd4p3.mexa64
!************************************************************************
...
! these are MT-CKD 4.3 or mods of that
      if (int(ckd) .eq. 43) then
        print *,'the MT_CKD version = ',int(ckd)
!        print *,int(idgas),int(nfreq),fstep,int(nlay),self,for,
!     $          int(whichlayer)
        print *,'calling calcon_mtckd_43_loc .....'
        call calcon_loc_mtckd_43_wrap(raZ,int(idgas),int(nfreq),raFreq, &
               fstep,int(nlay),raT,raP,raPP,raA,self,for,int(whichlayer))
      else
        print *,'the CKD version = ',int(ckd)
        print *,'calconwater_loc_ckd4p3 code needs <43>'
