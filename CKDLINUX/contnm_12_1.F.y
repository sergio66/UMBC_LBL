c copied from /strowdata1/shared/sergio/IR_NIR_VIS_UV_RTcodes/LBLRTM/MT_CKD2.5/cntnm/src_sergio/contnm_sergio.f
...
C
C
      SUBROUTINE CONTNM(JRAD,rXSelf,rXForn)                               F00010
C                                                                         F00020
      IMPLICIT REAL*8           (V)                                     ! F00030
      include 'max.inc'

      parameter (n_absrb=kCKDcoarse)
      parameter (nc=MaxLen)
C                                                                         F00040
...
c commented this out for run8.m
      if (x_vmr_h2o .ge. 1.0e-15) then
         wn2 = x_vmr_n2 * wtot    !! need to include N2 in WV continuum
       else
         !!only computing N2 continuum, so DO NOT CHANGE wn2!!!!
         wn2 = wk(22)
       endif
c       print *, ' this is wn2 ',wn2
...
C                                                                         F00740
               IF (JRAD.EQ.1) cself(j) = cself(j)*RADFN(VJ,XKT) *rXSelf   F00750
C              ---------------------------------------------------------
