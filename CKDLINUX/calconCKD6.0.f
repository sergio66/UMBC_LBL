c SAME as calconCKD4.0.f except it we fix the bump at 850 cm-1 (which is
c actually due to HNO3)

c SAME as calconCKD3.0.f except it applies the tuning multiplier both to
c SELF AND FOREIGN, as opposed to CKD3 which applies the tuning multiplier
c only to the SELF
...
c v6  does this from /asl/packages/sartaV104/Src_tuning/tunmlt_jan04deliv.txt
...
c copied from taro 
c  /asl/data/sarta_database/Data_IASI_may09/Coef/tunmlt_wcon_nte.txt  and then
c slightly massaged to make smoother transitions at 2165 cm-1, 2664 cm-1
c see fix_tunmlt_iasi_may09X.m
      iLenScottFile = 8461
      FNAME = '/home/sergio/SPECTRA/CKDLINUX/tunmlt_iasi_may09X.dat'
