...
c********************************************************
c New O2 continuum from LBLRTMv10 with arb mix ratio
      if (int(idgas) .eq. 7) then
        call calconOXYNEW(raZ,int(idgas),int(nfreq),raFreq,fstep,
     $                  int(nlay),raT,raP,raPP,raA,int(whichlayer))

c********************************************************
c New N2 continuum from LBLRTMv10 with arb mix ratio
      elseif (int(idgas) .eq. 22) then
        call calconNITNEW(raZ,int(idgas),int(nfreq),raFreq,fstep,
     $                  int(nlay),raT,raP,raPP,raA,int(whichlayer))
c********************************************************
c New O3 continuum from LBLRTMv10 with arb mix ratio
      elseif (int(idgas) .eq. 3) then
        call calconOZONENEW(raZ,int(idgas),int(nfreq),raFreq,fstep,
     $                  int(nlay),raT,raP,raPP,raA,int(whichlayer))
c********************************************************
      else
        print *,'your GasID = ',int(idgas)
        print *,'code needs 3,7,22 (oxygen,nitrogen)'
