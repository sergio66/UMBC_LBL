...
c   do the actual computations in a subroutine
      if (int(ckd) .eq. 00) then
          call calcon00(raZ,int(idgas),int(nfreq),raFreq,fstep,
     $        int(nlay),raT,raP,raPP,raA,self,for,int(whichlayer))
      elseif (int(ckd) .eq. 21) then
          call calcon21(raZ,int(idgas),int(nfreq),raFreq,fstep,
     $        int(nlay),raT,raP,raPP,raA,self,for,int(whichlayer))
      elseif (int(ckd) .eq. 23) then
          call calcon23(raZ,int(idgas),int(nfreq),raFreq,fstep,
     $        int(nlay),raT,raP,raPP,raA,self,for,int(whichlayer))
      else
        print *,'your CKD version = ',int(ckd)
        print *,'code needs 0,21,23'
