...
C
C
c modified by Sergio DeSouza-Machado, UMBC, July 2011
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

       SUBROUTINE CALCON_MTCKD_06_loc(raCON, IDGAS,NFREQ,raFreq,FSTEP,
...
c variables to read in scott's fixes
       REAL*8 aa1,raAIRS_WN(2378),aa3,aa4,raAIRS_FIX(2378),aa6,aa7,aa8,aa9
       INTEGER iIOUN,iErr
       CHARACTER*80 FNAME
...
C
C     The following modifications to the water vapor continuum 
c     arise from new analyses of
C     ARM measurements in the microwave and far-IR regions. Analyses of 
c     measurements in the microwave are based primarily on the two-channel MWR 
c     (23.8 and 31.4 GHz)
...
C     Mlawer and S. A. Clough). Measurements in the far-IR were from the 
c     AERI_ext at the NSA site, in the time surrounding and including the 
c     RHUBC-I campaign (J. Delamere and S. A. Clough).
C
C=======================================================================


c------------------------------------------------------------------------
c 1010 FORMAT('ERROR! number ',I5,' opening data file:',/,A80)
c      FNAME = '/home/sergio/SPECTRA/CKDLINUX/tunmlt_con1_14jul03.dat'
c      FNAME = '/home/sergio/SPECTRA/CKDLINUX/tunmlt_31oct03.dat'
c      FNAME = '/home/sergio/SPECTRA/CKDLINUX/tunmlt_jan04deliv.dat'
cc copied from taro
cc  /asl/data/sarta_database/Data_IASI_may09/Coef/tunmlt_wcon_nte.txt  and then
cc slightly massaged to make smoother transitions at 2165 cm-1, 2664 cm-1
cc see fix_tunmlt_iasi_may09X.m
c      FNAME = '/home/sergio/SPECTRA/CKDLINUX/tunmlt_iasi_may09X.dat'
c      iIOUN = 11
c      OPEN(UNIT=iIOUN,FILE=FNAME,STATUS='OLD',FORM='FORMATTED',
c     $     IOSTAT=IERR)
c      IF (IERR .NE. 0) THEN
c        write(*,1010) IERR, FNAME
c        STOP
c        ENDIF
c      DO iI = 1,2378
c        READ(iIOUN,*) aa1,raAIRS_WN(iI),aa3,aa4,raAIRS_FIX(iI),aa6,aa7,aa8,aa9
c        END DO
c      CLOSE(iIOUN)

c      CALL xlinear(raAIRS_WN,raAIRS_FIX,2378,raFreq,raScottMultiplier,
c     $             NFREQ)
c      DO iI=1,NFREQ
c        IF (raFreq(iI) .LT. raAIRS_WN(1)) THEN
c          raScottMultiplier(iI) = 1.0d0
c        ELSEIF (raFreq(iI) .GT. raAIRS_WN(2378)) THEN
c          raScottMultiplier(iI) = 1.0d0
c          END IF
c        END DO
