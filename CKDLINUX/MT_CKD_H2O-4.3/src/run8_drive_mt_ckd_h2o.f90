...
! test drives the mt_ckd_h2o module
   ...
   read *,ca_NML_FileIn
   write(*,'(A,A)') 'Will be reading input params from ',ca_NML_FileIn

   OPEN(UNIT=iIOUN, FILE=ca_NML_FileIn, STATUS="old", ACTION="read", IOSTAT=io_status)
...
!   istat= nf_create("mt_ckd_h2o_output.nc",nf_clobber,ncid) 
   istat= nf_create(caNC_FileOut,nf_clobber,ncid) 
